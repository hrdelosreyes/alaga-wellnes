  ...
  if (!data) {
    return (
      <div className="min-h-screen bg-[#F7F2EE]"><TherapistNav />
        <div className="max-w-lg mx-auto px-4 py-10 text-center text-[#6E5F55]">Couldn&rsquo;t load your history.</div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-[#F7F2EE]">
      ...
      <div className="max-w-lg mx-auto px-4 py-6 flex flex-col gap-5">
        <div>
          ...
          <p className="text-sm text-[#6E5F55] mt-1">Your past sessions and declined requests.</p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-white rounded-2xl border border-[#EDE5DF] p-4 text-center">
            ...
            <p className="text-[10px] text-[#6E5F55] mt-1">Completed</p>
          </div>
          <div className="bg-white rounded-2xl border border-[#EDE5DF] p-4 text-center">
            <p className="text-xl font-bold text-[#2C2420]">{formatPrice(summary.totalEarned)}</p>
            <p className="text-[10px] text-[#6E5F55] mt-1">Earned (all time)</p>
          </div>
          <div className="bg-white rounded-2xl border border-[#EDE5DF] p-4 text-center">
            <p className="text-xl font-bold text-[#C4714A]">{summary.cancelledCount + summary.declinedCount}</p>
            <p className="text-[10px] text-[#6E5F55] mt-1">Cancelled / declined</p>
          </div>
        </div>
          ...
          {([['sessions', `Sessions (${jobs.length})`], ['declined', `Declined (${declined.length})`]] as const).map(([id, label]) => (
            <button
              ...
              className={cn(
                ...
                tab === id ? 'bg-[#2C2420] text-white border-[#2C2420]' : 'bg-white border-[#EDE5DF] text-[#6E5F55]',
              )}
            >
              {label}
            </button>
        ...
        {tab === 'sessions' && (
          jobs.length === 0 ? (
            <div className="bg-white rounded-2xl border border-[#EDE5DF] p-8 text-center text-sm text-[#6E5F55]">
              ...
              {jobs.map(j => (
                <div key={j.id} className="bg-white rounded-2xl border border-[#EDE5DF] p-4">
                  <div className="flex items-start justify-between gap-2 mb-1.5">
                    ...
                    <span className={cn('text-xs font-semibold px-2.5 py-1 rounded-full capitalize shrink-0', STATUS_BADGE[j.status] ?? 'bg-[#F2EBE6] text-[#6E5F55]')}>
                      {j.status}
                    </span>
                  </div>
                  <p className="text-xs text-[#6E5F55]">{formatDate(j.booking_date)} · {formatTime(j.time_slot)}</p>
                  {j.customer_name && (
                    <p className="text-xs text-[#6E5F55] mt-0.5">{j.customer_name}</p>
                  )}
                  {j.address && (
                    <p className="text-xs text-[#6E5F55] mt-0.5 flex items-start gap-1"><MapPin size={11} className="mt-0.5 shrink-0" /><span className="truncate">{j.address}</span></p>
                  )}
                  <div className="mt-2.5 pt-2.5 border-t border-[#F2EBE6] flex items-center justify-between">
                    <span className="text-xs text-[#6E5F55]">{j.status === 'completed' ? 'Your payout' : 'No payout'}</span>
            ...
            <div className="bg-white rounded-2xl border border-[#EDE5DF] overflow-hidden divide-y divide-[#F2EBE6]">
              {declined.map((d, i) => (
                <div key={i} className="flex items-center justify-between gap-3 px-4 py-3.5">
                  ...
                  <span className="text-xs text-[#6E5F55] shrink-0">Declined {formatDate(d.declined_at.slice(0, 10))}</span>
                </div>
              ))}
            </div>
          )
        ...
        <p className="text-center text-[11px] text-[#6E5F55]">
