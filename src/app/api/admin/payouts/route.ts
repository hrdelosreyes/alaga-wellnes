...
export async function GET() {
  ...
  const [{ data: therapists }, { data: completed }, { data: payouts }] = await Promise.all([
    svc.from('therapists').select('id, name'),
  ...
  const rows = (therapists ?? [])
    .map(t => {
      ...
      return { therapist_id: t.id, name: t.name, earned, paidOut, pending: Math.max(0, earned - paidOut) }
    })
...
export async function POST(req: NextRequest) {
  ...
  const { therapistId, amount } = await req.json()
